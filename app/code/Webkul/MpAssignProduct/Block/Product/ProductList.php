    ...
    public function getAllProducts()
    {
        if (!$this->_productList) {
            ...
            if ($queryString != '') {
                ...
                /*$barcodeAttributeId = $this->greenlineHelper->getAttributeId('barcode', 4);
                $eavtable = 'catalog_product_entity_varchar';            
                $collection->getSelect()->joinLeft(
                    $eavtable.' as eavtable',
                    'e.entity_id = eavtable.entity_id AND eavtable.attribute_id = '.$barcodeAttributeId,
                    [
                        'barcode' => 'value',
                    ]
                );*/
                ...
                $collection->addFieldToFilter(
                    [
                        ...
                        ['attribute'=>'barcode','like'=> '%'.$queryString.'%']
                    ]
                );
                                
                $collection->addFieldToFilter('type_id', ['in' => $allowedTypes]);
                
                /*------ additional code for filter default Seller's Product start---*/
                ...
                )->where('cgf.seller_id = '.$defaultSellerId);
                /*------ additional code for filter default Seller's Product end---*/
