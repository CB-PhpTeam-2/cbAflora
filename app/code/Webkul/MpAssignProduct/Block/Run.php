...
class Run extends \Magento\Framework\View\Element\Template
{
            ...
            if (isset($assignProductData['error'])) {
                ...
                try {
                    if ($assignProductData['Parent Product Sku'] != '') {
                        ...
                        $product = $this->getProductByBarcode($assignProductData['Barcode']);
                    }
                    if ($product->getId()) {
                        ...
                        if ($productFromSameSeller) {
                            ...
                            Product Barcode is %1', $assignProductData['Barcode']));
                    ...
                    $wholeData[]['error'] =  __('Invalid Barcode: " %1"', $assignProductData['Barcode']);
                }
            }
    ...
    public function prepareData($product, $csvData, $sellerId)
    {
        try {
            ...
            if ($product->getTypeId() == 'configurable') {
                ...
                $newRow['product_condition'] = 1;
                $childProduct = $this->getProductByBarcode($csvData['Barcode']);
    ...
    public function getImagesData($assignProductData)
    {
        try {
            $wholedata = [];
            $data=[];
            $data['product']['Barcode'] = $assignProductData['Barcode'];
    ...
    public function getProductByBarcode($barcode)
    {
        ...
        $collection->addFieldToFilter(
                    [
                        ['attribute'=>'barcode','like'=> '%'.$barcode.'%']
                    ]
                );
