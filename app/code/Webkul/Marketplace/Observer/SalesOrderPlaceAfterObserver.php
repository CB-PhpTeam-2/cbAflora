...
class SalesOrderPlaceAfterObserver implements ObserverInterface
{
    ...
    public function orderPlacedOperations($order, $lastOrderId)
    {
        ...
        foreach ($sellerOrder as $info) {
            ...
            if($info['seller_id']){
                if($service_type == 2){
                    $pickup_service = $service_type;
                }
                $sellerObj= $this->_marketplaceHelper->getSellerDataBySellerId($info['seller_id']);
            ...
            $emailTempVariables['pickup_service'] = $pickup_service;
            $emailTempVariables['pickup_shop'] = $shop_title;
            $emailTempVariables['pickup_location'] = $pickup_location;

            $emailTempVariables['store_name'] = $order->getStore()->getName();
